...
fun EnhancedDocumentCameraPreview(
    ...
    LaunchedEffect(Unit) {
        scope.launch {
            val cameraManager = EnhancedCameraManager(context, lifecycleOwner)
            ...
            cameraManager.startCamera(
                ...
                // Process the captured frame only after delay
                processCameraFrame(bitmap, viewModel, documentSide, isProcessingComplete, cameraManager, isScanDelayActive)
            }
            
            // Collect preview state
            cameraManager.previewState.collect { previewInstance ->
                preview = previewInstance
            }
        }
        
        scope.launch {
            enhancedCameraManager?.isFocusStable?.collect { stable ->
                isFocusStable = stable
                android.util.Log.d("EnhancedCameraPreview", "Focus stable: $stable")
            }
        }
    }
